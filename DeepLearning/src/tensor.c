...
// size_t* size: an array containing the size of each dimension
// returns: a tensor of specified size filled with zeroes
tensor tensor_make(const size_t n, const size_t *size)
{
    tensor t = {0};
    t.n = n;
    // Creates n blocks of 'size_t' size
    t.size = n > 0 ? calloc(n, sizeof(size_t)) : 0;
    size_t i;
    // assigns the size for every dimension
    for(i = 0; i < n; ++i){
        t.size[i] = size[i];
    }
    size_t len = tensor_len(t);
    // t.data is a long array with enough blocks for (sum of each dimension and its size)
    // Ex: 2 dimensions, each with size 3 has total size of 6
...
tensor tensor_copy(tensor t)
{
    ...
    size_t original_len = tensor_len(t);
    size_t i;
...
// Returns a 2x2 matrix copy of the first (y_dim X x_dim) amount of data points
// tensor x: the tensor to copy from
// size_t y_dim: the y dimension to set, (or amount of rows)
// size_t x_dim: the x dimension to set, (or amount of columns)
tensor tensor_get_lim(tensor x, size_t y_dim, size_t x_dim) {
    size_t lim = y_dim * x_dim;
    tensor output = tensor_vmake(2, y_dim, x_dim);
    size_t i;
    for (i = 0; i < lim; ++i) {
...
// Performs e^(x) (in-place)
// tensor t: tensor to compute
// returns: tensor equal to e^(x)
void tensor_e_x_(tensor t) {
    size_t len = tensor_len(t);
    size_t i;
...
void tensor_simple_print(tensor t) {
    size_t i;
    printf("Dim = %ld, Size = (", t.n);
    for(i = 0; i < t.n; ++i) printf("%ld%s", t.size[i], (i<t.n-1)?", ":"");
    printf(")\n");
...
// Will sum numbers of a tensor across a dimension
// Ex. for a 3x4 matrix, `tensor_sum_dim(dim=0)` will sum columns resulting in a 1x4 matrix
// Ex. for a 3x4 matrix, `tensor_sum_dim(dim=1)` will sum rows resulting in a 1x3 matrix
