...
# deliverable 5.2
# DONE
def build_linear(x_vector, y_vector):
    '''
...
def nll_loss(logP, Y_tr):
    ...
    '''
    # output = []
    # for instance in logP:
    #     val = np.sum(np.dot(instance, -1)) / len(instance)
    #     output.append(val)
    # return np.sum(output) / len(logP)
    neg = -1 * logP
    print(logP[1])
    print(neg[1])
    print()
    ...
    print(neg[1][Y_tr[1]])
    specific_logP = []
    for idx, instance in enumerate(neg):
        specific_logP.append(instance[Y_tr[idx]])
    print(specific_logP[1])
    return np.mean(specific_logP)
