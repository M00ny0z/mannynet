...
class VanillaWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        for word in sentence:
            embedding_tensor = ag.Variable(torch.tensor([self.word_to_ix[word]]))
            curr_embedding = self.word_embeddings(embedding_tensor)
            embeds.append(curr_embedding)
        # END STUDENT
        return embeds
...
class BiLSTMWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        self.hidden = self.init_hidden()
        word_sequence = utils.prepare_sequence(sentence, self.word_to_ix)
        word_embeds = self.word_embeddings(word_sequence).view(len(sentence), 1, -1)
        embedding_sequence, self.hidden = self.lstm(word_embeds, self.hidden)
        return list(embedding_sequence)
...
class SuffixAndWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        for word in sentence:
            ...
            suffix_idx = [self.suff_to_ix[suffix_key]]
            word_embedding_tensor = torch.tensor(word_idx)
            suffix_embedding_tensor = torch.tensor(suffix_idx)
            word_embedding = self.word_embeddings(word_embedding_tensor)
            suffix_embedding = self.suffix_embeddings(suffix_embedding_tensor)
            embeds.append(torch.cat((word_embedding, suffix_embedding), dim=1))
        # END STUDENT
        return embeds
...
class FFCombiner(nn.Module):
    ...
    def __init__(self, embedding_dim):
        ...
        self.linear1 = nn.Linear(2 * embedding_dim, 1 * embedding_dim)
        self.linear2 = nn.Linear(1 * embedding_dim, 1 * embedding_dim)
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        out = self.linear1(torch.cat((head_embed[0], modifier_embed[0])))
        out = torch.tanh(out)
        out = self.linear2(out).reshape(1, -1)
        return out
...
class LSTMCombiner(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        # STUDENT
        # self.hidden = self.init_hidden()
        comb_tensor = torch.cat((head_embed, modifier_embed)).view(1, 1, -1)
        # print(comb_tensor.size())
        output, self.hidden = self.lstm(comb_tensor, self.hidden)
...
class FFActionChooser(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.linear1 = nn.Linear(1 * input_dim, 1 * input_dim)
        # 2. The second linear layer
        self.linear2 = nn.Linear(1 * input_dim, Actions.NUM_ACTIONS)
    ...
    def forward(self, inputs):
        ...
        """
        # STUDENT
        features = torch.cat(inputs, dim=-1).view(-1)
...
class LSTMActionChooser(nn.Module):
    ...
    def __init__(self, input_dim, num_layers, dropout):
        ...
        self.lstm = nn.LSTM(input_size=input_dim, hidden_size=input_dim, num_layers=num_layers, dropout=dropout)
        self.linear = nn.Linear(input_dim, Actions.NUM_ACTIONS)
    ...
    def forward(self, inputs):
        ...
        """
        # STUDENT
        features = torch.cat(inputs, dim=-1).view(1, 1, -1)
        lstm_out, self.hidden = self.lstm(features, self.hidden)
        lstm_out = lstm_out.view(1, -1)
        out_relu = F.relu(lstm_out)
        linear_out = self.linear(out_relu)
