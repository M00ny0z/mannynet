...
class ParserState:
    ...
    def _create_arc(self, head, modifier):
        ...
        """
        #STUDENT
        new_embedding = self.combiner(head.embedding, modifier.embedding)
        new_entry = StackEntry(head.headword, head.headword_pos, new_embedding)
        self.input_buffer.insert(0, new_entry)
    ...
    def _validate_action(self, action_to_do):
        ...
        if action_to_do == Actions.SHIFT:
            ...
            elif len(self.input_buffer) <= 2:
                return Actions.ARC_R
    
        if action_to_do == Actions.ARC_L or action_to_do == Actions.ARC_R:
            if len(self.stack) == 0:
                return Actions.SHIFT
        
        if action_to_do == Actions.ARC_L:
            if self.stack[-1].headword == ROOT_TOK:
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        while not parser_state.done_parsing():
            ...
            best_action = action_queue.popleft() if have_gold_actions else parser_state._validate_action(utils.argmax(log_probs))
            if best_action == 0:
                parser_state.shift()
            elif best_action == 1:
